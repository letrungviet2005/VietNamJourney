...
function CreateCampaign() {
  ...
  const handleAddOrganizationClick = () => {
    setOrganizations([...organizations, { organizationName: '', contactEmail: '' }]);
  };

  // Hàm xử lý thay đổi thông tin tổ chức
  const handleOrganizationNameChange = (index, e) => {
    const newOrganizations = [...organizations];
    newOrganizations[index].organizationName = e.target.value;
  ...
  const handleContactEmailChange = (index, e) => {
    const newOrganizations = [...organizations];
    newOrganizations[index].contactEmail = e.target.value;
    setOrganizations(newOrganizations);
  };
    ...
    <div className={cx("CreateCampaign")}>
      ...
      <div className={cx("row")}>
          ...
          {contacts.map((contact, index) => (
            <div key={index} className={cx("form")}>
              <div className={cx("form-group")}>
                <label htmlFor={`organizationName-${index}`}>
                  Tên tổ chức:
                </label>
                <input
                  type="text"
              ...
              <div className={cx("form-group")}>
                <label htmlFor={`contactEmail-${index}`}>Email liên hệ:</label>
                <input
                  type="email"
        ...
        <div className={cx("col-6")}>
          ...
          <button className={cx("contact-orgnazation")}>
            Thêm thông tin liên hệ
          </button>
        </div>
      </div>
