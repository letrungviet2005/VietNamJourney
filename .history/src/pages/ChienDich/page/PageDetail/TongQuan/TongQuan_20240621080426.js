...
function TongQuan({ campaign }) {
  const currentDate = new Date(); // Lấy ngày hiện tại
          const dateStart = new Date(response.data.dateStart); // Chuyển đổi ngày bắt đầu từ chuỗi
          const dateEnd = new Date(response.data.dateEnd); // Chuyển đổi ngày kết thúc từ chuỗi
    ...
    <div className={cx('TongQuan')}>
      ...
      <div className={cx('info')}>
        ...
        <div className={cx('buttons')}>
          <button className={cx('button-red', 'button')}>Bạn chưa đăng ký tham gia</button>
          <button className={cx('button-blue', 'button')}>Thông tin đang chờ duyệt</button>
          <button className={cx('button-green', 'button')}>Bạn là thành viên của chiến dịch</button>
        </div>
