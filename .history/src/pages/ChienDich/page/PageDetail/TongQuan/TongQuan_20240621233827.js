...
function TongQuan({ campaign }) {
  ...
  const [status, setStatus] = useState(null); // State lưu trữ trạng thái
  const [isLoading, setLoading] = useState(false); // State quản lý trạng thái loading
    ...
    const fetchStatus = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost/bwd/VietNamJourney/Server/ChienDich/GetStatus.php`,
          {
            method: "POST",
            headers: {
