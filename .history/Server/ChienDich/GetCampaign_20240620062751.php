...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $sql = "SELECT name, province, district, location, dateStart, dateEnd, totalMoney, moneyByVNJN, timeline, infoContact, infoOrganization, image, description, status) FROM campaign WHERE id = ?";
            ...
            foreach ($campaign as $key => $value) {
                ...
                if (is_string($value)) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                        ...
                        // Kiểm tra nếu giá trị là một đối tượng object
                        $object_value = unserialize($value); // Phương thức unserialize() có thể giúp giải nén đối tượng từ chuỗi
                        if ($object_value !== false && is_object($object_value)) {
                            // Chuyển đổi đối tượng thành mảng kết hợp
                            $campaign[$key] = (array) $object_value;
                        }
                    }
                }
            }
